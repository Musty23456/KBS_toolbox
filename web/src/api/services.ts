...
import type { Role, Submission, SubmissionStatus, ReviewStatus, SubmissionReview, SurveyDetail, SurveySummary, UserAccount, Device } from "./types";
...
export const surveysApi = {
  ...
  async update(
    id: string,
    payload: { title?: string; description?: string; questions?: any[]; sections?: any[]; groups?: any[]; assigned_enumerator_ids?: string[] }
  ): Promise<SurveyDetail> {
...
export const submissionsApi = {
  ...
  async create(payload: {
    ...
    answers: { question_id: string; value_text?: string | null }[];
  }): Promise<Submission> {
...
export const devicesApi = {
  async list(): Promise<Device[]> { const { data } = await apiClient.get("/api/devices"); return data; },
  async requestSync(deviceId: string): Promise<Device> { const { data } = await apiClient.post(`/api/devices/${deviceId}/request-sync`); return data; },
  async deactivate(deviceId: string): Promise<Device> { const { data } = await apiClient.post(`/api/devices/${deviceId}/deactivate`); return data; },
};

export const mapApi = {
  async submissionPoints(filters?: { survey_id?: string; status?: string }): Promise<import("./types").SubmissionMapPoint[]> {
    const { data } = await apiClient.get("/api/map/submissions", { params: filters });
    return data;
  },
};

export const exportsApi = {
  async download(format: "csv" | "json" | "xlsx" | "pdf", filters: { survey_id?: string; status?: string; submitted_by_id?: string; date_from?: string; date_to?: string } = {}) {
    const response = await apiClient.get(`/api/exports/${format}`, {
      params: filters,
      responseType: "blob",
    });
    ...
    link.href = url;
    link.download = `kbs-submissions.${format === "xlsx" ? "xlsx" : format}`;
...
export const analyticsApi = {
  async overview(filters: { survey_id?: string; date_from?: string; date_to?: string } = {}) {
    const { data } = await apiClient.get("/api/analytics/overview", { params: filters });
    return data;
  },
  async questions(surveyId: string, questionId?: string) {
    const { data } = await apiClient.get("/api/analytics/questions", {
...
export const translationsApi = {
  async languages(): Promise<{code: string; name: string}[]> {
    const { data } = await apiClient.get("/api/surveys/languages"); return data;
  },
  async list(surveyId: string): Promise<import("./types").Translation[]> {
    const { data } = await apiClient.get(`/api/surveys/${surveyId}/translations`); return data;
  },
  async upsert(surveyId: string, payload: Omit<import("./types").Translation, "id">): Promise<import("./types").Translation> {
    const { data } = await apiClient.put(`/api/surveys/${surveyId}/translations`, payload); return data;
  },
};
